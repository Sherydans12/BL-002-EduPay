...
export function validateNameTokenPreservation(
  ...
  if (!rawLegacy) {
    return { valid: false, reason: "El nombre original no es válido o está vacío." };
  }

  if (!rawFirst) {
    return { valid: false, reason: "El campo Nombres no puede estar vacío." };
  }

  if (!rawLast) {
    return { valid: false, reason: "El campo Apellidos no puede estar vacío." };
  }
  ...
  if (legacyTokens.length === 0) {
    return { valid: false, reason: "El nombre original no contiene palabras válidas." };
  }
